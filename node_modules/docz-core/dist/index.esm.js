...
import { readJsonSync, readJSON, outputFile, outputJSON, pathExistsSync, copy, ensureDir, copySync, removeSync, readFileSync, readJSONSync, outputJSONSync, existsSync } from 'fs-extra';
import { get, set } from 'env-dot-prop';
import humanize from 'humanize-string';
import titleize from 'titleize';
import { realpathSync } from 'fs';
import { join, resolve, parse, delimiter, isAbsolute, relative, extname, normalize, dirname } from 'path';
import { sync } from 'resolve';
import logger__default, { error, warn as warn$1 } from 'signale';
...
const getCoreVersions = () => async dep => {
  const depPath = join(root, '../../core', dep);
  const pkgJSONFilepath = join(depPath, 'package.json');
  const pkg = await readJSON(pkgJSONFilepath, {
    throws: false
  });
...
const createDeps = async ctx => {
  ...
  return {
    dependencies: Object.assign({}, dependencies, (await getDeps(REQUIRED_DEV_DEPS, ctx, pkg)), (await getCoreDeps(ctx, pkg))),
...
const copyAndModifyPkgJson = async ctx => {
  const filepath = join(root, 'package.json');
  const movePath = join(docz, 'package.json');
  const pkg = await readJSON(filepath, {
    throws: false
  });
...
const writeEslintRc = async ({
  ...
  const filepath = join(docz, '.eslintrc');
  await outputJSON(filepath, {
    extends: 'react-app'
  });
};
...
const installDeps = async ({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const result = spawn$1.sync('yarn', ['--production=false'], {
      stdio: 'inherit'
    });
...
const watchGatsbyThemeFiles = args => {
  ...
  const copy = filepath => {
    ...
    const dest = resolve(docz, filepath);
    copySync(src, dest);
  };
...
function getPropsOnCache() {
  ...
  if (_isEmpty(cache)) {
    warn$1('Any cache was found with your props definitions');
    warn$1("We'll parse your components to get props from them");
    warn$1('Depending on your components, this could take while...');
    return [];
  }
...
const getPattern = config => {
  ...
  return ignore.map(entry => typeof entry === 'string' && `!**/${entry}`).filter(Boolean).concat([join(src, ts ? '**/*.{ts,tsx,js,jsx,mjs}' : '**/*.{js,jsx,mjs}'), '!**/node_modules', '!**/doczrc.js']);
};

...
import { createContext, Component, createElement, useContext, Fragment, useEffect, useMemo, useRef, useState, memo } from 'react';
import { Playground as Playground$1 } from 'docz-components';
...
const Playground = ({
  ...
  return createElement(Playground$1, {
    className: className,
    style: style,
    wrapper: Wrapper,
    scope: __scope,
...
const Props = ({
  ...
  const found = stateProps && stateProps.length > 0 && stateProps.find(item => item.key.includes(`/${componentName}.`) || item.key === filename);
